...
func (s *Sales) Ring(ctx context.Context, actor Actor, in SaleInput) (SaleResult, error) {
	...
	err := s.db.InTx(ctx, func(tx *sql.Tx) error {
		...
		sale, err := q.CreateSale(ctx, gen.CreateSaleParams{
			...
			// PPN stays zero until the tax engine lands (TASKS 5.3). Nothing
			// here computes a rate (INV-4).
			PpnIdr: 0, TotalIdr: int64(priced.total), CogsIdr: int64(totalCOGS),
		...
		for _, d := range drawn {
			line, err := q.CreateSaleLine(ctx, gen.CreateSaleLineParams{
				...
				NetIdr:           int64(d.priced.net), CogsIdr: int64(d.result.COGS), CreatedAt: now,
			})
			if err != nil {
				return wrapWrite(err)
			}
		...
		if len(payments) == 0 && !in.IsCredit {
			payments = []PaymentInputLine{{Method: "TUNAI", AmountIDR: priced.total}}
		}
		...
		out.COGS = totalCOGS
		out.Margin = priced.total.Sub(totalCOGS)
...
func (s *Sales) CreateReturn(ctx context.Context, actor Actor, in SaleReturnInput) (SaleReturnResult, error) {
	...
	err := s.db.InTx(ctx, func(tx *sql.Tx) error {
		...
		for i, rl := range in.Lines {
			...
			// the line -- net of both discounts -- by the same prefix
			// difference used for cost, so returning a line in pieces refunds
			// exactly what it took in.
			net := money.IDR(line.NetIdr)
			refund := net.MulRatio(already+rl.Qty, line.Qty).Sub(net.MulRatio(already, line.Qty))
			...
			pending = append(pending, pendingLine{line: line, qty: rl.Qty, refund: refund, cogs: lineCOGS.Neg()})
		...
		ret, err := q.CreateSaleReturn(ctx, gen.CreateSaleReturnParams{
			...
			CogsReversedIdr: int64(totalCOGS), RefundMethod: in.RefundMethod,
			CreatedBy: nilIfEmpty(actor.UserID), CreatedAt: now,
		})
		if err != nil {
			return wrapWrite(err)
		...
		for _, p := range pending {
			row, err := q.CreateSaleReturnLine(ctx, gen.CreateSaleReturnLineParams{
				...
				Qty: p.qty, RefundIdr: int64(p.refund), CogsReversedIdr: int64(p.cogs), CreatedAt: now,
			})
			if err != nil {
				return wrapWrite(err)
			}
